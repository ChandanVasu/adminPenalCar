...
export default function Model() {
    ...
    const handleAddModel = async (e) => {
        ...
        try {
            const response = await fetch(modelApiEndpoint, {
                ...
                body: JSON.stringify({ model: newModel, image: newImage, make: selectedMake })
            });
            if (!response.ok) throw new Error("Network response was not ok");
            setNewModel("");
            setNewImage("");
    ...
    const handleUpdateModel = async () => {
        if (!selectedModel || !newModel.trim() || !newImage.trim() || !selectedMake) {
        ...
        <div>
            <div className="flex justify-between p-6 space-x-6">
                <div className="w-1/2 p-4 rounded-lg shadow-md h-min">
                    ...
                    <form onSubmit={selectedModel ? (e) => { e.preventDefault(); handleUpdateModel(); } : handleAddModel}>
                        <Select
                            label="Make"
                            variant="bordered"
                ...
                <div className="w-1/2 p-4 rounded-lg shadow-md">
                    ...
                    <ul className="list-disc pl-5">
                        {modelData.map((modelItem) => (
                            <li key={modelItem._id} className="flex justify-between items-center px-5 py-2 mb-4 rounded-md bg-slate-50">
                                <img src={modelItem.image} alt={modelItem.model} className="bg-white w-14 h-14 object-cover mr-4 rounded-full shadow-md p-2" />
